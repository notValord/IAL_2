...
ht_init(test_table);
ht_item_t *h = ht_search(test_table, "Ethereum");
        if(h == NULL){
            printf("Searchol som NULL\n");
        }
        else{
            printf("Searchol som %s\n", h->key);
        }
ENDTEST
...
ht_insert(test_table, "Ethereum", 3208.67);
    ...
    if(h == NULL){
        ...
        printf("Searchol som %s %f\n", h->key, h->value);
    }
    if(h->next == NULL){
        printf("Ten item ukazuje na NULL\n");
    }
    else{
        printf("Neukazuje na NULL\n");
    }
ENDTEST
...
ht_init(test_table);
INSERT_TEST_DATA(test_table)
    ht_item_t *h = ht_search(test_table, "Terra");
    if(h == NULL){
        printf("Searchol som NULL\n");
    }
    else{
